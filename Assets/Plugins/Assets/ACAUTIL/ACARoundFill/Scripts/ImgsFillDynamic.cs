...
public class ImgsFillDynamic : MonoBehaviour
{
    ...
    void SetTextFactor()
    {
        ...
        float leftFactor = 100 - textFactor;
        if (this.TxtValue != null)
            if (textFactor == 0) TxtValue.gameObject.SetActive(false);
            this.TxtValue.text = string.Format("{0}{1}", textFactor.ToString("0"), this.TailText);

        if(LeftValue != null) 
        {
            if (leftFactor == 0) LeftValue.gameObject.SetActive(false);
            this.LeftValue.text = string.Format("{0}{1}", leftFactor.ToString("0"), this.TailText);
        }
    }
